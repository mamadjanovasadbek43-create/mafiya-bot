...
async def yordam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "*Admin:*\n/berpul @username miqdor — Pul berish\n/berdiamond @username miqdor — Olmos berish",
        parse_mode="Markdown"
    )
...
async def qoshilish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result == "already":
        ...
        return
    await update.message.reply_text(f"✅ *{user.first_name}* qo'shildi! 👥 Jami: {len(game.players)}", parse_mode="Markdown")
...
async def join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result == "already":
        ...
        return
    await context.bot.send_message(chat_id, f"✅ *{user.first_name}* qo'shildi! 👥 Jami: {len(game.players)}", parse_mode="Markdown")
...
async def boshlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🎭 *O'YIN BOSHLANDI!*\n\n👥 O'yinchilar:\n{players_list}\n\n🌙 Kecha boshlanmoqda!", parse_mode="Markdown")
    for player_id, player in game.players.items():
        role_info = game.get_role_info(player['role'])
        try:
            await context.bot.send_message(player_id, f"🎭 *Rolingiz: {role_info['name']}*\n\n{role_info['description']}\n\n{role_info['ability']}", parse_mode="Markdown")
        except:
            pass
...
async def start_night(update, context, chat_id):
    ...
    await context.bot.send_message(chat_id,
        ...
        "Tun tugadi: /tunguvoh", parse_mode="Markdown")
...
async def rolim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(user.id, f"🎭 *Rolingiz: {role_info['name']}*\n\n{role_info['description']}\n\n{role_info['ability']}", parse_mode="Markdown")
...
async def ovoz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🗳 *{user.first_name}* → *{target['name']}*", parse_mode="Markdown")
...
async def end_voting(update, context, chat_id):
    ...
    if result:
        ...
        role_info = game.get_role_info(player['role'])
        await context.bot.send_message(chat_id, f"⚖️ *{player['name']}* chiqarildi!\n🎭 Rol: {role_info['name']}\n\n🌙 Kecha...", parse_mode="Markdown")
    else:
...
async def end_game(context, chat_id, winner):
    ...
    await context.bot.send_message(chat_id, "🏆 G'oliblar: 💎+10, 🪙+50\n\nYangi o'yin: /yangioyun")
    del games[chat_id]
